...
def logout():
    ...
    if not oidc_cfg.get('end_provider_session', False):
        log.debug('oidc.end_provider_session is disabled – skipping provider logout, showing local logged-out page.')
    ...
    try:
        ...
        log.warning('Failed to load OIDC server metadata for logout redirect – falling back to local logout page.')
        end_session_endpoint = None

    if not end_session_endpoint:
        log.debug('No end_session_endpoint in OIDC metadata – showing local logged-out page.')
        return redirect(url_for('auth.logged_out'))
    ...
    log.debug('OIDC RP-Initiated Logout – end_session_endpoint: %s', end_session_endpoint)
    log.debug('OIDC RP-Initiated Logout – post_logout_redirect_uri: %s', post_logout_uri)
