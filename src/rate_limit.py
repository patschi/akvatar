...
class _RateLimiter:
    ...
    def check(self, ip: str, cost: int) -> tuple[bool, int]:
        ...
        with self._lock:
            ...
            if total_points + cost > self._cfg.max_points:
                ...
                if entries:
                    ...
                    retry_after = max(
                        1, int(expires_at - now) + 1 + self._cfg.eviction_interval
                    )
                else:
                    retry_after = self._cfg.eviction_interval + 1
                log.debug(
                    "[%s]: denied %s (%d/%d points, retry_after=%ds).",
                    self._cfg.name,
                    ip,
                    total_points,
                    self._cfg.max_points,
                    retry_after,
                )
        ...
        if current_tier > prev_tier and total_points < self._cfg.max_points:
            ...
            log.debug(
                "[%s]: %s at %d%% of limit (%d/%d points in window).",
                self._cfg.name,
                ip,
                pct,
                total_points,
            ...
            with self._lock:
                ...
                if pruned > 0:
                    ...
                    if entries:
                        ...
                        log.debug(
                            "[%s]: pruned %d expired entries for %s, %d remain.",
                            self._cfg.name,
                            pruned,
                            ip,
                            len(entries),
                        ...
                        log.debug(
                            "[%s]: evicted %s (all %d entries expired).",
                            self._cfg.name,
                            ip,
                            pruned,
                        )
...
class _RateLimitManager:
    ...
    def __init__(self, rate_cfg: dict) -> None:
        ...
        for key in (ENDPOINT_AVATARS, ENDPOINT_METADATA):
            ...
            log.info(
                "Rate limiter [%s]: %d points / %d s window.",
                key,
                limiter_cfg.max_points,
                limiter_cfg.window,
            )
        ...
        if self._whitelist:
            log.info(
                "Rate limiting whitelist: %s",
                ", ".join(str(n) for n in self._whitelist),
            )
        else:
    ...
    def check(self, endpoint_type: str, ip: str, cost: int) -> tuple[bool, int]:
        ...
        if not allowed:
            log.warning(
                "Rate limit exceeded [%s]: ip=%s (retry_after=%ds).",
                endpoint_type,
                ip,
                retry_after,
            )
    ...
    def start_eviction_thread(self) -> None:
        ...
        if not self._limiters:
            ...
            return
        t = threading.Thread(
            target=self._eviction_loop, name="ratelimit-evict", daemon=True
        )
        t.start()
        log.debug(
            "Rate limit eviction thread started (interval=%ds).",
...
def init_rate_limiting(app: Flask) -> None:
    ...
    def _check_rate_limit():
        ...
        allowed, retry_after = _manager.check(
            endpoint_type, request.remote_addr, COST_NORMAL
        )
        if allowed:
            return None
        ...
        if response.status_code == 404:
            ...
            if endpoint_type is not None:
                _manager.add_points(
                    endpoint_type,
                    request.remote_addr,
                    _manager.cost_not_found - COST_NORMAL,
                )
        return response
