...
def is_enabled() -> bool:
    ...
    return ldap_cfg.get('enabled', False)
...
def update_thumbnail(username: str, jpeg_bytes: bytes) -> None:
    ...
    skip_verify = ldap_cfg.get('skip_cert_verify', False)
    log.debug('Connecting to AD server %s:%s (SSL=%s, skip_cert_verify=%s).', ldap_cfg['server'], ldap_cfg['port'], ldap_cfg['use_ssl'], skip_verify)

    tls_config = None
    if ldap_cfg['use_ssl'] and skip_verify:
        tls_config = ldap3.Tls(validate=ssl.CERT_NONE)

    server = ldap3.Server(ldap_cfg['server'], port=ldap_cfg['port'], use_ssl=ldap_cfg['use_ssl'], tls=tls_config, get_info=ldap3.ALL)
    conn = ldap3.Connection(server, user=ldap_cfg['bind_dn'], password=ldap_cfg['bind_password'], auto_bind=True)
    ...
    try:
        ...
        max_bytes = ldap_cfg.get('max_thumbnail_bytes', 102400)
        if len(jpeg_bytes) > max_bytes:
            raise ValueError(f'Thumbnail JPEG is {len(jpeg_bytes)} bytes, exceeding the limit of {max_bytes} bytes.')
        log.debug('Thumbnail size OK: %d bytes (limit %d).', len(jpeg_bytes), max_bytes)
