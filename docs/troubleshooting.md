...
1. **Authentik avatar source setting.** In the Authentik admin panel, ensure the user's avatar source is set to use the
   ...
   `avatar`). Authentik may be using a different source (Gravatar, initials) that takes precedence. Note that a
   companion pure-ID attribute is written alongside the URL under
   [`authentik.avatar_id_attribute`](configuration.md#authentikavatar_id_attribute) (default: `avatar_id`); Authentik
   itself should still be pointed at the URL attribute, not the ID attribute.
