...
The Avatar Updater uses the Authentik Admin API to read user attributes and set the avatar URL on the user object. This
requires an API token with the appropriate permissions.

## What the application does with the API

1. **Resolve user PK:** At login time, the app calls `GET /api/v3/core/users/?username=<username>` to find the user's
   numeric primary key (PK). This PK is used as a stable identifier for all subsequent operations.
...
3. **Write avatar URL and avatar ID:** The app calls `PATCH /api/v3/core/users/<pk>/` to set
   `attributes.avatar` (or whichever attribute is configured via `authentik.avatar_attribute`) to the public URL of
   the uploaded avatar, and `attributes.avatar_id` (or whichever attribute is configured via
   `authentik.avatar_id_attribute`) to the pure avatar ID (filename base, no URL/hostname/extension). Both
   attributes are written in a single PATCH and are kept in lockstep on upload, reset, and rollback.
...
3. Fill in:
    ...
    - **User**: assign it to a user that has permissions to read and write user attributes (
      see [Required permissions](#required-permissions) below)
...
The simplest approach is to assign the token to an **admin** user. For a least-privilege setup, create a dedicated
service account in Authentik and grant it only the permissions listed above through Authentik's RBAC system.

## Security considerations

- **Treat the token as a secret.** It grants write access to user attributes. Do not commit it to version control.
- **Scope the token to the minimum required permissions.** If your Authentik version supports fine-grained API
  permissions, restrict the token to user attribute reads and writes only.
- **Rotate the token periodically** if your security policy requires it. After rotating, update `authentik.api_token` in
  `config.yml` and restart the application.
